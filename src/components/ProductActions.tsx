...
export function ProductActions({ productId }: { productId: string }) {
  ...
  return (
    <div className="mt-6 flex flex-wrap gap-3">
      ...
      <button onClick={() => cart.add(productId, quantity)} className="rounded-md bg-turquoise px-5 py-3 font-extrabold text-white">Ajouter au panier</button>
      <Link href="/commande" onClick={() => cart.add(productId, quantity)} className="rounded-md bg-navy px-5 py-3 font-extrabold text-white">Commander maintenant</Link>
    </div>
  );
}
