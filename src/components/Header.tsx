...
import { ChevronDown, FileText, MapPin, Menu, Phone, Share2, ShoppingCart, Truck, User, X } from "lucide-react";
    ...
    <header className="sticky top-0 z-50 bg-white shadow-sm">
      <div className="bg-navy-deep text-white">
        <div className="container-page flex items-center justify-between gap-3 py-2 text-xs">
          ...
          <button
            ...
            className="inline-flex shrink-0 items-center gap-2 rounded-md border border-white/20 px-3 py-2 font-extrabold md:hidden"
          ...
          <div className="border-t border-white/10 md:hidden">
            <div className="container-page flex gap-2 py-2">
              {socialLinks.map((item) => (
                <Link key={item.label} href={item.href} onClick={() => setSocialOpen(false)} className="flex-1 rounded-md bg-white/10 px-3 py-2 text-center text-xs font-extrabold text-white">
                  {item.label}
                </Link>
              ))}
            </div>
      ...
      <div className="container-page flex items-center gap-3 py-3 md:gap-5 md:py-4">
        ...
        <div className="ml-auto flex shrink-0 items-center gap-2 md:hidden">
          ...
          <button onClick={() => { setCartOpen(true); setSocialOpen(false); }} className="relative grid h-11 w-11 place-items-center rounded-md border border-line" aria-label="Panier">
        ...
        <button className="grid h-11 w-11 shrink-0 place-items-center rounded-md border border-line md:hidden" onClick={() => { setMenuOpen(!menuOpen); setSocialOpen(false); }} aria-label="Menu">
          {menuOpen ? <X /> : <Menu />}
        </button>
      </div>
      ...
      <nav className={`${menuOpen ? "block" : "hidden"} bg-navy text-white md:block`}>
        <div className="container-page flex flex-col md:flex-row md:items-center">
          {links.map((link) => {
            ...
            return (
              <Link key={link.href} href={link.href} onClick={() => { setMenuOpen(false); setSocialOpen(false); }} className={`flex items-center gap-1 border-b border-white/10 px-3 py-3 text-sm font-bold transition hover:text-turquoise-light md:border-b-0 ${active ? "text-turquoise-light" : ""}`}>
